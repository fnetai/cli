...
export default async ({
  ...
  params.name = params.name || "rust";
  params.entry = params.entry || "app/esm";
  params.version = params.version || "0.1.0";

  params.title = params.title || atom?.doc?.title || params.name || atom?.doc?.name || "Rust App";
  params.package_name = params.package_name || params.name;
  params.author = params.author || atom?.doc?.author || "Flownet";
  params.description = params.description || atom?.doc?.description || "Rust App built with Flownet";
  ...
  // template dir
  const templateDir = resolveTemplatePath('./template/deploy/to-rust');
  ...
  const srcDistDir = path.resolve(path.join(src, `dist`, params.entry));
  ...
  const destSrcDir = path.resolve(outDir, './dist');
  ...
  shResult = await fnetShellJs(`cargo build`, { cwd: outDir });
  if (shResult.code !== 0) throw new Error('Couldn\'t build project.');

  shResult = await fnetShellJs(`cargo build --release`, { cwd: outDir });
  if (shResult.code !== 0) throw new Error('Couldn\'t build project.');
}
