...
class API {
    ...
    _initRouter() {
        ...
        router.post('/workflow/builder/create', this._buildHandler.bind(this));
        router.post('/workflow/builder/status', this._statusHandler.bind(this));
        router.post('/workflow/builder/network', this._networkHandler.bind(this));
    ...
    _networkHandler(req, res) {
        this._network({ ...req.body })
            .then(response => {
                res.status(200).send(response);
            })
            .catch(error => {
    ...
    async _build(reqContext) {
        ...
        if (!id) throw new Error('Workflow Id is not defined.');

        const buildId =nanoid(24);

        const file = path.resolve(__dirname, 'builder/wf-cli.js');
    ...
    async _network(reqContext) {
        ...
        const command = `node ${file} build --id=${id} --buildId=${buildId} --mode=bpmn`;

        shelljs.exec(command, { cwd: this.#wdir});

        const buildKey = "BUILD:" + buildId;

        const status = await this._cache_get(buildKey);
