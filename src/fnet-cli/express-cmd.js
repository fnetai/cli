...
async function handleExpressList(argv) {
  try {
    ...
    // Set default type filter to 'fnet' if not specified and --all is not used
    ...
    console.log(chalk.blue(`\nTotal: ${projects.length} fnet ${projects.length === 1 ? 'project' : 'projects'}`));
  } catch (error) {
...
/**
 * Check if a project matches this CLI's type (fnet)
 */
function isOwnType(projectPath, projectName) {
  return projectName.startsWith('fnet-') || resolveProjectFile(projectPath, 'fnet', { migrate: false }) !== null;
}
